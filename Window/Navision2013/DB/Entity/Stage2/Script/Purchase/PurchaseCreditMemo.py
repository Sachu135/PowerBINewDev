...
sqlCtx, spark = getSparkConfig("local[*]", "Stage2:PurchaseCreditMemo")
def purchase_PurchaseCreditMemo():
    ...
    for dbe in config["DbEntities"]:
        if dbe['ActiveInactive']=='true' and  dbe['Location']==DBEntity:
            ...
            try:
                ...
                logger = Logger()
                pcml = spark.read.format("parquet").load(STAGE1_PATH+"/Purch_ Cr_ Memo Line")
                pcmh = spark.read.format("parquet").load(STAGE1_PATH+"/Purch_ Cr_ Memo Hdr_")
                gps =spark.read.format("parquet").load(STAGE1_PATH+"/General Posting Setup")
                pld = spark.read.format("parquet").load(STAGE1_PATH+"/Posted Str Order Line Details")
                VE = spark.read.format("parquet").load(STAGE1_PATH+"/Value Entry")
                dse=spark.read.format("parquet").load(STAGE2_PATH+"/"+"Masters/DSE")
                PurchaseInvoice=spark.read.format("parquet").load(STAGE2_PATH+"/"+"Purchase/PurchaseInvoice")    
