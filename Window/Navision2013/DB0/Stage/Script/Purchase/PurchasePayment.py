...
apmode = 'append'                           
st = dt.datetime.now()
conf = SparkConf().setMaster("local[*]").setAppName("PurchasePayment").\
...
try:
    ...
    for d in range(len(DBList)):  
        ...
        for i in range(NoofRows): 
                ...
                Path = Abs_Path+"/"+DBName+"/"+EntityName+"\\Stage2\\ParquetData\\Purchase\PurchasePayment"
    ...
    log_df = spark.createDataFrame(log_dict, logger.getSchema())
    log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
print('Purchase_ PurchasePayment completed: ' + str((dt.datetime.now()-st).total_seconds()))     
