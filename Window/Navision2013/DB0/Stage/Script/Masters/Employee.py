...
try:
    ...
    for d in range(len(DBList)):  
        ...
        for i in range(NoofRows): 
                ...
                Path = Abs_Path+"/"+DBName+"/"+EntityName+"\\Stage2\\ParquetData\\Masters\Employee"
                ...
                if os.path.exists(Path):
                    ...
                    print("Employee "+DBName+EntityName+" Does not exist")
    ...
    log_dict = logger.getSuccessLoggedRecord("Masters.Employee", DB0, " ", finalDF.count(), len(finalDF.columns), IDEorBatch)
    ...
    try:
        ...
        IDEorBatch = "IDLE"
    DBE=DBName+EntityName
    os.system("spark-submit "+Kockpit_Path+"\Email.py 1 Employee "+CompanyName+" "+" "+str(exc_traceback.tb_lineno)+"")   
    log_dict = logger.getErrorLoggedRecord('Masters.Employee', DB0, " " , str(ex), exc_traceback.tb_lineno, IDEorBatch)
    log_df = spark.createDataFrame(log_dict, logger.getSchema())
    log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
print('Masters_ Employee completed: ' + str((dt.datetime.now()-st).total_seconds()))     
