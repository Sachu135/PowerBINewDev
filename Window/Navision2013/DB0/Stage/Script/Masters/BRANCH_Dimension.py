...
apmode = 'append'                           
st = dt.datetime.now()
sqlCtx, spark = getSparkConfig("local[*]", "Stage:BRANCH_Dimension")
    ...
    try:
        ...
        for d in range(len(DBList)):  
            ...
            for i in range(NoofRows): 
                    ...
                    Path = Abs_Path+"/"+DBName+"/"+EntityName+"\\Stage2\\ParquetData\\Masters\BRANCH_Dimension"
        ...
        log_df = spark.createDataFrame(log_dict, logger.getSchema())
        log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
    print('Masters_ BRANCH_Dimension completed: ' + str((dt.datetime.now()-st).total_seconds()))     
