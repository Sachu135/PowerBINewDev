...
apmode = 'append'                           
st = dt.datetime.now()
sqlCtx, spark = getSparkConfig("local[*]", "Stage:Item")
def masters_Item():
    ...
    try:
        ...
        for d in range(len(DBList)):  
            ...
            for i in range(NoofRows): 
                    ...
                    Path = Abs_Path+"/"+DBName+"/"+EntityName+"\\Stage2\\ParquetData\\Masters\Item"
                    ...
                    if os.path.exists(Path):
                        ...
                        print("Item "+DBName+EntityName+" Does not exist")
        ...
        log_dict = logger.getSuccessLoggedRecord("Masters.Item", DB0, " ", finalDF.count(), len(finalDF.columns), IDEorBatch)
        ...
        log_df = spark.createDataFrame(log_dict, logger.getSchema())
        log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
    print('Masters_ Item completed: ' + str((dt.datetime.now()-st).total_seconds()))     
