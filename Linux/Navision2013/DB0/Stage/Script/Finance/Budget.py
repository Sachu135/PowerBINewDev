...
apmode = 'append'                           
st = dt.datetime.now()
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "StageDB0:Finance-Budget")
def finance_Budget():
    ...
    try:
        ...
        for d in range(len(DBList)):  
            ...
            for i in range(NoofRows): 
                    ...
                    Path = HDFS_PATH+DIR_PATH+"/"+DBName+"/"+EntityName+"/Stage2/ParquetData/Finance/Budget"
                    ...
                    if(fe):
                        ...
                        print("Budget "+DBName+EntityName+" Does not exist")
        finalDF.write.jdbc(url=PostgresDbInfo.PostgresUrl , table="Finance.Budget", mode=owmode, properties=PostgresDbInfo.props)
        logger.endExecution()
        try:
        ...
        log_dict = logger.getSuccessLoggedRecord("Finance.Budget", DB0, "", finalDF.count(), len(finalDF.columns), IDEorBatch)
        ...
        try:
            ...
            IDEorBatch = "IDLE"
        DBE=DBName+EntityName
        os.system("spark-submit "+Kockpit_Path+"\Email.py 1 Budget "+CompanyName+" "" "+str(exc_traceback.tb_lineno)+"")   
        log_dict = logger.getErrorLoggedRecord('Finance.Budget', DB0, "" , str(ex), exc_traceback.tb_lineno, IDEorBatch)
        log_df = spark.createDataFrame(log_dict, logger.getSchema())
        log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
        print('Finance Budget completed: ' + str((dt.datetime.now()-st).total_seconds())) 
