...
apmode = 'append'                           
st = dt.datetime.now()
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "StageDB0:Finance-ProfitLoss")
    ...
    try:
        ...
        for d in range(len(DBList)):  
            ...
            for i in range(NoofRows): 
                    ...
                    Path = HDFS_PATH+DIR_PATH+"/"+DBName+"/"+EntityName+"/Stage2/ParquetData/Finance/ProfitLoss"
                    ...
                    if(fe):
                        ...
                        print("ProfitLoss "+DBName+EntityName+" Does not exist")
        ...
        log_dict = logger.getSuccessLoggedRecord("Finance.ProfitLoss", DB0, "", finalDF.count(), len(finalDF.columns), IDEorBatch)
        ...
        try:
            ...
            IDEorBatch = "IDLE"
        DBE=DBName+EntityName
        os.system("spark-submit "+Kockpit_Path+"\Email.py 1 ProfitLoss "+CompanyName+" "" "+str(exc_traceback.tb_lineno)+"")   
        log_dict = logger.getErrorLoggedRecord('Finance.ProfitLoss', DB0, "" , str(ex), exc_traceback.tb_lineno, IDEorBatch)
        log_df = spark.createDataFrame(log_dict, logger.getSchema())
        log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
    print('Finance ProfitLoss completed: ' + str((dt.datetime.now()-st).total_seconds()))   
