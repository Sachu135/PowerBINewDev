...
apmode = 'append'                           
st = dt.datetime.now()
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "StageDB0:Sales-ManualCOGS")
def sales_ManualCOGS():
    ...
    try:
        ...
        for d in range(len(DBList)):  
            ...
            for i in range(NoofRows): 
                    ...
                    Path = HDFS_PATH+DIR_PATH+"/"+DBName+"/"+EntityName+"/Stage2/ParquetData/Sales/ManualCOGS"
                    ...
                    if(fe):
                        ...
                        print("ManualCOGS "+DBName+EntityName+" Does not exist")
        ...
        try:
            ...
            IDEorBatch = "IDLE"
        DBE=DBName+EntityName
        os.system("spark-submit "+Kockpit_Path+"\Email.py 1 ManualCOGS "+CompanyName+" "" "+str(exc_traceback.tb_lineno)+"")   
        log_dict = logger.getErrorLoggedRecord('Sales.ManualCOGS', DB0, "" , str(ex), exc_traceback.tb_lineno, IDEorBatch)
        log_df = spark.createDataFrame(log_dict, logger.getSchema())
        log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
    print('Sales ManualCOGS completed: ' + str((dt.datetime.now()-st).total_seconds()))   
