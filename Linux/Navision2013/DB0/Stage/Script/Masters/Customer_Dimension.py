...
apmode = 'append'                           
st = dt.datetime.now()
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "StageDB0:Masters-CUSTOMER_Dimension")
def masters_CUSTOMER_Dimension():
    ...
    try:
        ...
        for d in range(len(DBList)):  
            ...
            for i in range(NoofRows): 
                    ...
                    Path = HDFS_PATH+DIR_PATH+"/"+DBName+"/"+EntityName+"/Stage2/ParquetData/Masters/CUSTOMER_Dimension"
                    ...
                    if(fe):
                        ...
                        print("CUSTOMER_Dimension "+DBName+EntityName+" Does not exist")
        finalDF.write.jdbc(url=PostgresDbInfo.PostgresUrl , table="Masters.CUSTOMER_Dimension", mode=owmode, properties=PostgresDbInfo.props)
        logger.endExecution()
        try:
        ...
        log_dict = logger.getSuccessLoggedRecord("Masters.CUSTOMER_Dimension", DB0, "", finalDF.count(), len(finalDF.columns), IDEorBatch)
        ...
        try:
            ...
            IDEorBatch = "IDLE"
        DBE=DBName+EntityName
        os.system("spark-submit "+Kockpit_Path+"\Email.py 1 CUSTOMER_Dimension "+CompanyName+" "" "+str(exc_traceback.tb_lineno)+"")   
        log_dict = logger.getErrorLoggedRecord('Masters.CUSTOMER_Dimension', DB0, "" , str(ex), exc_traceback.tb_lineno, IDEorBatch)
        log_df = spark.createDataFrame(log_dict, logger.getSchema())
        log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
    print('masters CUSTOMER_Dimension completed: ' + str((dt.datetime.now()-st).total_seconds()))
