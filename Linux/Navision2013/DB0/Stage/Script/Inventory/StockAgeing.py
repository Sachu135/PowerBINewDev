...
try:
    ...
    for d in range(len(DBList)):  
        ...
        for i in range(NoofRows): 
                ...
                Path = HDFS_PATH+DIR_PATH+"/"+DBName+"/"+EntityName+"/Stage2/ParquetData/Finance/StockAgeing"
    ...
    log_dict = logger.getSuccessLoggedRecord("Finance.StockAgeing", DB0, "", finalDF.count(), len(finalDF.columns), IDEorBatch)
    ...
    log_dict = logger.getErrorLoggedRecord('Finance.StockAgeing', DB0, "" , str(ex), exc_traceback.tb_lineno, IDEorBatch)
    log_df = spark.createDataFrame(log_dict, logger.getSchema())
    log_df.write.jdbc(url=PostgresDbInfo.PostgresUrl, table="logs.logs", mode='append', properties=PostgresDbInfo.props)        
print('Finance StockAgeing completed: ' + str((dt.datetime.now()-st).total_seconds()))        