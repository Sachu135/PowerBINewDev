...
def updateSchemaMetadata(dirnameInHDFS, tblNameInHDFS, tableColumns):
    try:
        ...
        for col in tableColumns:
            fullName = "delta.`" + Kockpit_Path+"/" +DBName+"/" +EntityName+"/" +"Stage1/ParquetData"+  "/" + tblNameInHDFS + "`"
...
def task(entity, Table, sqlurl, hdfsLocation,Schema_DF):
    ...
    try:
        ...
        tableQuery = "(SELECT top 1 "+temp+" FROM ["+table_name+"] WITH (NOLOCK)) AS GetColumns" #WHERE 1=0
                ...
                if 'Key' in Table and 'CheckOn' in Table:
                    try:
                        ...
                        if(schemaStr == hdfsSchemaStr):
                            ...
                            tableDataDF = tableDataDF.select([F.col(col).alias(col.replace(' ', '').replace('(', '').replace(')', '')) for col in tableDataDF.columns])
                            tableDataDF.cache()
                            dtTable=DeltaTable.forPath(spark, HDFS_Path)#(spark,stage1/db1e1)
                            dtTable.vacuum(0.1)
                            mergeConditionsList = []
                            keyNames = []
                            modifiedKeyNames = []
...
for entityObj in ac.config["DbEntities"]:
    ...
    HDFS_Schema_DF = sqlCtx.sparkSession.read.format("delta").load(Kockpit_Path+"/" +DBName+"/" +EntityName+"/" +"Stage1/ParquetData" + "/Schema")
