...
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "Stage2:Masters-Vendor")
def masters_vendor():
    for dbe in config["DbEntities"]:
        if dbe['ActiveInactive']=='true' and  dbe['Location']==DBEntity:
            ...
            try:        
                logger = Logger()
                finalDF=spark.read.format("delta").load(STAGE1_PATH+"/Vendor" )
                ...
                result_df = result_df.select([F.col(col).alias(col.replace(")","")) for col in result_df.columns])
                result_df.write.option("maxRecordsPerFile", 10000).format("delta").mode("overwrite").option("overwriteSchema", "true").save(STAGE2_PATH+"/"+"Masters/Vendor")
