...
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "Stage2:Finance-MISPNL")
def finance_MISPNL():
    for dbe in config["DbEntities"]:
        if dbe['ActiveInactive']=='true' and  dbe['Location']==DBEntity:
            ...
            try:
                logger = Logger()
                Acc_Sche =spark.read.format("delta").load(STAGE1_PATH+"/Acc_ Schedule Line")
                COA = spark.read.format("delta").load(STAGE1_PATH+"/G_L Account")
                df = spark.read.format("delta").load(STAGE1_Configurator_Path+"/tblCompanyName")
