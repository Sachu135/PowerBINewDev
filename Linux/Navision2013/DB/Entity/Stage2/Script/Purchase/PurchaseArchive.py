...
sqlCtx,spark=getSparkConfig(SPARK_MASTER, "Stage2:Purchase-PurchaseArchive")
def purchase_PurchaseArchive():
    for dbe in config["DbEntities"]:
        if dbe['ActiveInactive']=='true' and  dbe['Location']==DBEntity:
            ...
            try:  
                logger = Logger()
                pah = spark.read.format("delta").load(STAGE1_PATH+"/Purchase Header Archive")
                ph=spark.read.format("delta").load(STAGE1_PATH+"/Purchase Header")
                pal = spark.read.format("delta").load(STAGE1_PATH+"/Purchase Line Archive").drop("DBName","EntityName","PostingDate")
                ...
                Purchase.write.option("maxRecordsPerFile", 10000).format("delta").mode("overwrite").option("overwriteSchema", "true").save(STAGE2_PATH+"/"+"Purchase/PurchaseArchive")   
                logger.endExecution()
